import type { EvalBenchmarkRubric, EvalTestCaseContent } from '@lobechat/types';
import { describe, expect, it } from 'vitest';

import { evaluate } from '../src';

const equalsRubric: EvalBenchmarkRubric = {
  config: { value: '' },
  id: 'r1',
  name: 'Exact Match',
  type: 'equals',
  weight: 1,
};

describe('evaluate', () => {
  it('should pass when actual matches expected', () => {
    const testCase: EvalTestCaseContent = { expected: '42', input: 'What is 6*7?' };
    const result = evaluate('42', testCase, [equalsRubric]);
    expect(result.passed).toBe(true);
    expect(result.score).toBe(1);
  });

  it('should fail when actual does not match', () => {
    const testCase: EvalTestCaseContent = { expected: '42', input: 'What is 6*7?' };
    const result = evaluate('41', testCase, [equalsRubric]);
    expect(result.passed).toBe(false);
    expect(result.score).toBe(0);
  });

  it('should handle multi-candidate expected (JSON array)', () => {
    const testCase: EvalTestCaseContent = {
      expected: JSON.stringify(['孙悟空', '悟空', '齐天大圣']),
      input: '西游记主角是谁?',
    };
    const result = evaluate('悟空', testCase, [equalsRubric]);
    expect(result.passed).toBe(true);
  });

  it('should use extractor from options', () => {
    const testCase: EvalTestCaseContent = {
      choices: ['0', '1', '2', '3'],
      expected: '1',
      input: 'Find all c in Z_3...',
    };
    const result = evaluate('The answer is B', testCase, [equalsRubric], {
      extractor: { type: 'choice-index' },
    });
    expect(result.passed).toBe(true);
    expect(result.score).toBe(1);
  });

  it('should use extractor from rubric over options', () => {
    const rubricWithExtractor: EvalBenchmarkRubric = {
      ...equalsRubric,
      extractor: { type: 'delimiter', delimiter: '####' },
    };
    const testCase: EvalTestCaseContent = { expected: '9', input: 'Calculate...' };
    const result = evaluate('blah blah #### 9', testCase, [rubricWithExtractor]);
    expect(result.passed).toBe(true);
  });

  it('should compute weighted score across rubrics', () => {
    const rubrics: EvalBenchmarkRubric[] = [
      { ...equalsRubric, id: 'r1', weight: 2 },
      { ...equalsRubric, id: 'r2', type: 'contains', weight: 1 },
    ];
    const testCase: EvalTestCaseContent = { expected: '42', input: '...' };
    // equals fails (actual != expected), contains passes (actual contains '42')
    const result = evaluate('The answer is 42', testCase, rubrics);
    // equals: 0 * 2 = 0, contains: 1 * 1 = 1, total = 1/3 ≈ 0.33
    expect(result.score).toBeCloseTo(1 / 3, 2);
    expect(result.passed).toBe(false); // below 0.6 threshold
  });

  it('should return empty results for no rubrics', () => {
    const testCase: EvalTestCaseContent = { expected: '42', input: '...' };
    const result = evaluate('42', testCase, []);
    expect(result.passed).toBe(false);
    expect(result.rubricResults).toHaveLength(0);
  });

  it('should respect custom passThreshold', () => {
    const testCase: EvalTestCaseContent = { expected: '42', input: '...' };
    const rubrics: EvalBenchmarkRubric[] = [
      { ...equalsRubric, id: 'r1', weight: 1 },
      { ...equalsRubric, id: 'r2', type: 'contains', weight: 1 },
    ];
    // equals fails, contains passes → score = 0.5
    const result = evaluate('The answer is 42', testCase, rubrics, { passThreshold: 0.5 });
    expect(result.passed).toBe(true);
  });
});

describe('evaluate - MMLU end-to-end', () => {
  it('should correctly evaluate MMLU-style question', () => {
    const testCase: EvalTestCaseContent = {
      choices: ['0', '1', '2', '3'],
      expected: '1',
      input: 'Find all c in Z_3 such that Z_3[x]/(x^2 + c) is a field.',
    };

    const rubrics: EvalBenchmarkRubric[] = [
      {
        config: { value: '' },
        id: 'mmlu-match',
        name: 'Choice Match',
        type: 'equals',
        weight: 1,
      },
    ];

    // Agent says "B" → extractor maps to index 1 → matches expected "1"
    const result = evaluate('The answer is B', testCase, rubrics, {
      extractor: { type: 'choice-index' },
      passThreshold: 0.6,
    });

    expect(result.passed).toBe(true);
    expect(result.score).toBe(1);
    expect(result.rubricResults[0].passed).toBe(true);
  });

  it('should fail when agent gives wrong answer', () => {
    const testCase: EvalTestCaseContent = {
      choices: ['0', '1', '2', '3'],
      expected: '1',
      input: 'Find all c in Z_3...',
    };

    const result = evaluate('I think the answer is C', testCase, [equalsRubric], {
      extractor: { type: 'choice-index' },
    });

    expect(result.passed).toBe(false); // C → 2, expected 1
  });

  it('should handle MMLU with verbose reasoning before answer', () => {
    const testCase: EvalTestCaseContent = {
      choices: ['True, True', 'False, False', 'True, False', 'False, True'],
      expected: '2',
      input: 'Statement 1 | Every element of a group generates a cyclic subgroup...',
    };

    const result = evaluate(
      'Let me think step by step.\nStatement 1 is true because...\nStatement 2 is false because S_10 has 10! elements.\nTherefore the answer is C.',
      testCase,
      [equalsRubric],
      { extractor: { type: 'choice-index' } },
    );

    expect(result.passed).toBe(true);
    expect(result.score).toBe(1);
  });
});

describe('evaluate - GSM8K end-to-end', () => {
  const numericRubric: EvalBenchmarkRubric = {
    config: { tolerance: 0.01, value: 0 },
    id: 'gsm8k-numeric',
    name: 'Numeric Match',
    type: 'numeric',
    weight: 1,
  };

  it('should extract answer after #### delimiter and match numerically', () => {
    const testCase: EvalTestCaseContent = {
      expected: '9',
      input: 'Janet sells 16-3-4=<<16-3-4=9>>9 duck eggs. How many?',
    };

    const result = evaluate(
      'Janet has 16 eggs. She eats 3 and bakes 4. So 16-3-4=9 eggs remain.\n\nThe answer is 9.',
      testCase,
      [{
        ...numericRubric,
        extractor: { type: 'last-line' },
      }],
    );

    expect(result.passed).toBe(true);
  });

  it('should handle GSM8K delimiter extraction', () => {
    const testCase: EvalTestCaseContent = {
      expected: '42',
      input: 'A store sells...',
    };

    const result = evaluate(
      'First we calculate... then we add... #### 42',
      testCase,
      [{
        ...numericRubric,
        extractor: { type: 'delimiter', delimiter: '####' },
      }],
    );

    expect(result.passed).toBe(true);
  });

  it('should handle decimal tolerance', () => {
    const testCase: EvalTestCaseContent = {
      expected: '3.14',
      input: 'What is pi to 2 decimal places?',
    };

    const result = evaluate(
      '3.14159',
      testCase,
      [{ ...numericRubric, config: { tolerance: 0.01, value: 3.14 } }],
    );

    expect(result.passed).toBe(true);
  });
});

describe('evaluate - browsecomp-zh / xbench style', () => {
  it('should match with contains for short answer in long output', () => {
    const containsRubric: EvalBenchmarkRubric = {
      config: { value: '' },
      id: 'contains-match',
      name: 'Contains Match',
      type: 'contains',
      weight: 1,
    };
    const testCase: EvalTestCaseContent = {
      expected: '161.27元',
      input: '某产品的价格是多少?',
    };

    const result = evaluate(
      '根据查询结果，该产品的售价为161.27元，目前有货。',
      testCase,
      [containsRubric],
    );

    expect(result.passed).toBe(true);
  });

  it('should handle multi-candidate Chinese answers', () => {
    const testCase: EvalTestCaseContent = {
      expected: JSON.stringify(['孙悟空', '悟空', '齐天大圣', '美猴王']),
      input: '西游记中大闹天宫的是谁?',
    };

    // Test with different valid answers
    expect(evaluate('齐天大圣', testCase, [equalsRubric]).passed).toBe(true);
    expect(evaluate('美猴王', testCase, [equalsRubric]).passed).toBe(true);
    expect(evaluate('猪八戒', testCase, [equalsRubric]).passed).toBe(false);
  });

  it('should handle xbench style with single round answer', () => {
    const testCase: EvalTestCaseContent = {
      expected: '1轮',
      input: '某比赛第几轮?',
    };

    const result = evaluate('1轮', testCase, [equalsRubric]);
    expect(result.passed).toBe(true);
  });
});

describe('evaluate - edge cases', () => {
  it('should handle empty actual output', () => {
    const testCase: EvalTestCaseContent = { expected: '42', input: '...' };
    const result = evaluate('', testCase, [equalsRubric]);
    expect(result.passed).toBe(false);
  });

  it('should handle undefined expected', () => {
    const testCase: EvalTestCaseContent = { input: '...' };
    const result = evaluate('anything', testCase, [equalsRubric]);
    // empty string vs 'anything' → fails
    expect(result.passed).toBe(false);
  });

  it('should handle whitespace-only output with extractor', () => {
    const testCase: EvalTestCaseContent = { expected: '1', input: '...' };
    const result = evaluate('   \n  \n  ', testCase, [equalsRubric], {
      extractor: { type: 'last-line' },
    });
    expect(result.passed).toBe(false);
  });

  it('should handle multiple rubrics with different extractors', () => {
    const rubrics: EvalBenchmarkRubric[] = [
      {
        config: { value: '' },
        extractor: { type: 'choice-index' },
        id: 'choice',
        name: 'Choice',
        type: 'equals',
        weight: 1,
      },
      {
        config: { value: '' },
        id: 'raw-contains',
        name: 'Raw Contains',
        type: 'contains',
        weight: 1,
      },
    ];
    const testCase: EvalTestCaseContent = {
      expected: '1',
      input: '...',
    };

    // "B" → choice-index extracts "1" → equals "1" ✓
    // raw output "The answer is B" contains "1"? No → ✗
    const result = evaluate('The answer is B', testCase, rubrics);
    expect(result.score).toBeCloseTo(0.5, 2);
    expect(result.rubricResults[0].passed).toBe(true);
    expect(result.rubricResults[1].passed).toBe(false);
  });
});
